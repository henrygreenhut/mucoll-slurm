...
class SimCopyTests(unittest.TestCase):
    ...
    def test_training_raw_time_cut_is_applied_when_copying(self):
        class Position:
            x = y = z = 0.0
        ...
        template = [Hit(-10.0), Hit(9_999_999.0), Hit(10_000_000.0), Hit(1e12)]
        hits = {name: [copy.copy(hit) for hit in template]
                for _, name in writer.COLLECTIONS.values()}
        ...
        event = {
            "_hit_selection": writer.TRAINING_RAW_TIME_SELECTION,
            "sources": {polarity: [{"path": f"{polarity}.root", "entry": 0}]
                        for polarity in writer.POLARITIES},
        }
        output = {short: Collection() for short in writer.COLLECTIONS}
        ...
        for short in writer.COLLECTIONS:
            self.assertEqual(
                [hit.time for hit in output[short]], [-10.0, 9_999_999.0] * 2)
            self.assertTrue(all(hit.overlay for hit in output[short]))
