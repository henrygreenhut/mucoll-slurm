...
# Build the norm42 store on OSCAR using the ORIGINAL, unmodified
# gen_libtest_make_store.py -- no deduplication needed here, since
# bib-v3p0-fmt2-norm42-RandomRot/GEN IS genuinely the 42x-cloned/rotated
# data (confirmed via inspect_gen_rotation.py earlier). Only the
# reconstruction (norm1) side needed the dedup workaround.
#
...
python gen_libtest_make_store.py \
    ...
    --output /oscar/scratch/$USER/mucoll/stores/gen_norm42_${POLARITY}.h5 \
    --workers 16

echo "done -> /oscar/scratch/$USER/mucoll/stores/gen_norm42_${POLARITY}.h5"
ls -lh /oscar/scratch/$USER/mucoll/stores/gen_norm42_${POLARITY}.h5
