...
for root_path in root_paths:
    path = Path(root_path)
    plot_prefix = "__".join(path.parts[-3:]).replace(".edm4hep.root", "").replace(".root", "")
...
#Before looping through events you should obtain all the arrays outside of the loop, if not it will take a long time 
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_trackmultiplicity.pdf")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_clustermultiplicity.pdf")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_pfomultiplicity.pdf")
    plt.close()
...
#This is its own category since it needs different loops
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_hitmultiplicity.pdf")
    plt.close()
...
#I don't believe tracks have energy (makes sense)
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_clusenergy.pdf")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_pfoenergy.pdf")
    plt.close()
#################################################################
...
#From this you can easily find theta distribution too
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_trackphi_0.pdf")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_clusterphi.pdf")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{plot_prefix}__basicplot_pfophi.pdf")
    plt.close()
